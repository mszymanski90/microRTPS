...
int main( void )
{
	...
	//microRTPS_Register(&mRTPS, 1, sizeof(Mess));
...
void vApplicationTickHook( void )
{
	...
	if( ulTicksSinceLastDisplay >= mainCHECK_DELAY )
	{
		...
		{
			sprintf(xMessage.msg, "ERROR IN GEN Q");
		}
		else if( xAreQueuePeekTasksStillRunning() != pdTRUE )
		{
			sprintf(xMessage.msg, "ERROR IN PEEK Q");
		}
		else if( xAreBlockingQueuesStillRunning() != pdTRUE )
		{
			sprintf(xMessage.msg, "ERROR IN BLOCK Q");
		}
		else if( xAreBlockTimeTestTasksStillRunning() != pdTRUE )
		{
			sprintf(xMessage.msg, "ERROR IN BLOCK TIME");
		}
	    else if( xAreSemaphoreTasksStillRunning() != pdTRUE )
	    {
	    	sprintf(xMessage.msg, "ERROR IN SEMAPHORE");
	    }
	    else if( xArePollingQueuesStillRunning() != pdTRUE )
	    {
	    	sprintf(xMessage.msg, "ERROR IN POLL Q");
	    }
	    else if( xIsCreateTaskStillRunning() != pdTRUE )
	    {
	    	sprintf(xMessage.msg, "ERROR IN CREATE");
	    }
	    else if( xAreIntegerMathsTaskStillRunning() != pdTRUE )
	    {
	    	sprintf(xMessage.msg, "ERROR IN MATH");
	    }
	    else if( xAreRecursiveMutexTasksStillRunning() != pdTRUE )
	    {
	    	sprintf(xMessage.msg, "ERROR IN REC MUTEX");
	    }
		else if( ulIdleError != pdFALSE )
		{
			sprintf(xMessage.msg, "ERROR IN HOOK");
		}
		else if( xAreIntQueueTasksStillRunning() != pdTRUE )
		{
			sprintf(xMessage.msg, "ERROR IN INT QUEUE");
		}
...
void ( *vOLEDClear )( void ) = NULL;
	...
	for( ;; )
	{
		...
		for(i=0; i<LCD_MSG_MAX; i++)
		{
			cMessage[i] = xMessage.msg[i];
		}
